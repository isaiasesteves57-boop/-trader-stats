...
(function () {
  ...
  // Uma única data é aplicada a todos os jogos daquele lote como
  // "data_jogo". Também registramos "dataImportacao" (data em que o
  // lote foi importado/lido), que é sempre a data atual e nunca deve
  // ser confundida com a data do jogo.
  function aplicarDataDoLote(itens, data) {
    const dataFinal = data || hojeISO();
    const dataImportacao = hojeISO();
    return itens.map((op) => Object.assign({}, op, { data: dataFinal, dataImportacao }));
  }
  ...
  // Janela padrão da tela principal: somente jogos de hoje e de amanhã.
  // Jogos com data_jogo anterior a hoje desaparecem automaticamente
  // da tela principal (mas continuam existindo no armazenamento e no
  // Histórico/Mapa, que consultam a lista completa).
  function estaNaJanelaPrincipal(op) {
    const d = opDataEfetiva(op);
    return d === hojeISO() || d === amanhaISO();
  }
  ...
  // Substitui window.confirm(): em muitos PWAs instalados (modo standalone,
  // principalmente em Android/TWA e iOS "Adicionar à Tela de Início") o
  // diálogo nativo confirm()/alert() não aparece e simplesmente retorna
  // falso na hora — fazendo o clique parecer que "não faz nada". Este modal
  // roda 100% dentro da própria página, então funciona igual em qualquer
  // ambiente (navegador normal ou app instalado).
  function confirmarAcao(mensagem, textoBotaoConfirmar) {
    return new Promise((resolve) => {
      ...
      overlay.innerHTML = `
        <div class="modal-sheet">
          ...
          <p style="margin:0 0 18px;color:var(--text-1);">${escapeHtml(mensagem)}</p>
          <div style="display:flex;gap:10px;">
            <button type="button" class="btn btn-ghost btn-block" data-acao="cancelar">CANCELAR</button>
            <button type="button" class="btn btn-red btn-block" data-acao="confirmar">${escapeHtml(textoBotaoConfirmar || 'CONFIRMAR')}</button>
          </div>
        </div>`;
      ...
      overlay.addEventListener('click', (e) => {
        if (e.target === overlay) return concluir(false);
        const btnAcao = e.target.closest('[data-acao]');
        if (!btnAcao) return;
        concluir(btnAcao.dataset.acao === 'confirmar');
      });
    });
  }
  ...
  function renderDashboard() {
    ...
    // A tela principal só mostra jogos de hoje/amanhã — jogos antigos
    // somem automaticamente daqui (mas seguem no Histórico/Mapa).
    const listaJanela = lista.filter(estaNaJanelaPrincipal);
    const filtrada = ordenarPorHorario(filtrarListaPorStatus(listaJanela, filtroStatusAtual));
  ...
  async function executarAcaoCard(acao, id) {
    ...
    switch (acao) {
      ...
      case 'red':
        ...
        break;
      case 'excluir': {
        const ok = await confirmarAcao('Excluir esta oportunidade?', 'EXCLUIR');
        if (ok) {
...
// ---------------------------------------------------------------
  ...
  // ---------------------------------------------------------------
  // Limpeza manual de jogos antigos / de um dia específico
  // ---------------------------------------------------------------

  const btnLimparAntigos = $('#btn-limpar-antigos');
  if (btnLimparAntigos) {
    btnLimparAntigos.addEventListener('click', async () => {
      const hoje = hojeISO();
      const ok = await confirmarAcao(
        'Isso vai remover da lista principal e do armazenamento todos os jogos com data anterior a hoje. Continuar?',
        'APAGAR ANTIGOS'
      );
      if (!ok) return;
      const removidos = RadarStorage.removerJogosAntigos(hoje);
      mostrarToast(removidos > 0 ? `${removidos} jogo(s) antigo(s) removido(s)` : 'Nenhum jogo antigo para remover');
      renderTudo();
    });
  }

  const btnLimparDia = $('#btn-limpar-dia');
  if (btnLimparDia) {
    btnLimparDia.addEventListener('click', () => {
      $('#input-data-limpar-dia').value = hojeISO();
      abrirModal('modal-limpar-dia');
    });
  }

  const btnConfirmarLimparDia = $('#btn-confirmar-limpar-dia');
  if (btnConfirmarLimparDia) {
    btnConfirmarLimparDia.addEventListener('click', async () => {
      const dataEscolhida = $('#input-data-limpar-dia').value;
      if (!dataEscolhida) {
        mostrarToast('Escolha uma data para apagar.');
        return;
      }
      const ok = await confirmarAcao(`Isso vai remover todos os jogos com data ${dataEscolhida}. Continuar?`, 'APAGAR');
      if (!ok) return;
      const removidos = RadarStorage.removerJogosPorData(dataEscolhida);
      mostrarToast(removidos > 0 ? `${removidos} jogo(s) removido(s)` : 'Nenhum jogo encontrado nessa data');
      fecharModal('modal-limpar-dia');
      renderTudo();
    });
  }
  ...
  $('#btn-interpretar-texto').addEventListener('click', () => {
    const dataLote = $('#input-data-lote-texto').value;
    if (!dataLote) {
      mostrarToast('Escolha a data dos jogos antes de interpretar o texto.');
      return;
    }
  ...
  $('#btn-interpretar-lay-zebra-texto').addEventListener('click', () => {
    const dataLote = $('#input-data-lote-lay-zebra').value;
    if (!dataLote) {
      mostrarToast('Escolha a data dos jogos antes de interpretar o texto.');
      return;
    }
  ...
  const zonaUpload = $('#upload-zone');
  const inputImagem = $('#input-imagem');

  if (zonaUpload && inputImagem) {
    ...
    inputImagem.addEventListener('change', async (e) => {
      ...
      const dataLoteEscolhida = $('#input-data-lote-imagem')?.value;
      if (!dataLoteEscolhida) {
        mostrarToast('Escolha a data dos jogos desta imagem antes de importar.');
        e.target.value = '';
        return;
      }

      // Mostra a imagem IMEDIATAMENTE após a seleção. Não esperamos o OCR
      // terminar para confirmar visualmente que o arquivo foi carregado.
      const previewUrl = URL.createObjectURL(file);
      $('#ocr-preview').innerHTML = `
        <div style="margin-top:10px;">
          <div style="font-size:12px;color:var(--text-2);margin-bottom:6px;">🖼️ Imagem selecionada</div>
          <img src="${previewUrl}" style="display:block;max-width:100%;max-height:420px;margin:0 auto;border-radius:10px;object-fit:contain;" />
        </div>`;
      $('#ocr-status').textContent = '⏳ Preparando leitura da imagem...';

      try {
        // Compatibilidade: versões antigas tinham a opção de 2 colunas.
        // Se ela ainda existir no HTML/cache, usamos seu valor; se não existir,
        // o leitor normal é usado automaticamente. Nunca acessamos .checked de null.
        const checkboxDuasColunas = $('#chk-ocr-duas-colunas');
        const usarDuasColunas = !!(checkboxDuasColunas && checkboxDuasColunas.checked);
        const lerFn = usarDuasColunas && RadarOCR.readTwoColumnImage
          ? RadarOCR.readTwoColumnImage
          : RadarOCR.readImage;

        const { textoBruto, dataUrlProcessado } = await lerFn(file, (p) => {
          $('#ocr-status').textContent = `⏳ Lendo imagem (OCR)... ${p}%`;
        });

        // Troca o preview original pelo processado somente depois que ele existir.
        if (dataUrlProcessado) {
          $('#ocr-preview').innerHTML = `
            <div style="margin-top:10px;">
              <div style="font-size:12px;color:var(--text-2);margin-bottom:6px;">✅ Imagem carregada e processada</div>
              <img src="${dataUrlProcessado}" style="display:block;max-width:100%;max-height:420px;margin:0 auto;border-radius:10px;object-fit:contain;" />
            </div>`;
        }

        const estrategiaLote = $('#input-estrategia-lote-imagem')?.value || '';
        let resultado = RadarParser.parseSmartText(textoBruto);
        resultado = aplicarDataDoLote(resultado, dataLoteEscolhida);
        resultado = aplicarEstrategiaDoLote(resultado, estrategiaLote);
        $('#ocr-status').textContent = '';

        if (resultado.length === 0) {
          $('#ocr-status').innerHTML = `Não consegui identificar oportunidades automaticamente. Texto lido:<br/><em style="color:var(--text-1)">${escapeHtml(textoBruto).slice(0, 600)}</em><br/>A imagem foi carregada corretamente. Você pode revisar/cadastrar manualmente.`;
          return;
        }
        ...
        console.error(err);
        $('#ocr-status').textContent = `Erro ao ler imagem: ${err && err.message ? err.message : err}`;
      } finally {
  ...
  function salvarCandidato(op) {
    ...
    if (!limpo.dataImportacao) limpo.dataImportacao = hojeISO();
  ...
  $('#btn-limpar-dados').addEventListener('click', async () => {
    const ok = await confirmarAcao('Isso vai apagar TODAS as oportunidades salvas neste dispositivo. Continuar?', 'APAGAR TUDO');
    if (ok) {
  ...
  $('#lista-metodos').addEventListener('click', async (e) => {
    ...
    if (btn.dataset.metodoAction === 'editar') {
      ...
      const ok = await confirmarAcao('Excluir este método?', 'EXCLUIR');
      if (ok) {
  ...
  $('#lista-estrategias-ajustes').addEventListener('click', async (e) => {
    ...
    if (btn.dataset.estrategiaAction === 'editar') {
      ...
      const ok = await confirmarAcao('Excluir esta estratégia?', 'EXCLUIR');
      if (ok) {
  ...
  function registrarServiceWorker() {
    if ('serviceWorker' in navigator) {
      ...
      if (location.protocol === 'http:' || location.protocol === 'https:') {
        navigator.serviceWorker.register('service-worker.js?v=6', { updateViaCache: 'none' }).catch((err) => {
