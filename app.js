...
(function () {
  ...
  $('#btn-interpretar-texto').addEventListener('click', () => {
    ...
    const resultado = RadarParser.parseText(texto);
  ...
  $('#input-imagem').addEventListener('change', async (e) => {
    ...
    $('#ocr-status').textContent = '⏳ Lendo imagem (OCR)...';
    $('#ocr-preview').innerHTML = '';

    try {
      const duasColunas = $('#chk-ocr-duas-colunas').checked;
      const lerFn = duasColunas ? RadarOCR.readTwoColumnImage : RadarOCR.readImage;
      const { textoBruto, dataUrlProcessado } = await lerFn(file, (p) => {
        $('#ocr-status').textContent = `⏳ Lendo imagem (OCR)... ${p}%`;
      });

      $('#ocr-preview').innerHTML = `<img src="${dataUrlProcessado}" style="max-width:100%; border-radius:10px; margin-top:10px;" />`;

      const resultado = RadarParser.parseText(textoBruto);
      $('#ocr-status').textContent = '';
  ...
  $('#btn-salvar-todas').addEventListener('click', () => {
    ...
    mostrarToast(`${candidatosRevisao.length} oportunidade(s) salva(s)`);
    candidatosRevisao = [];
  ...
  function salvarCandidato(op) {
    ...
    RadarStorage.addOpportunity(limpo);
  }
