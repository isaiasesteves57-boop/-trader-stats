...
// ---------------------------------------------------------------
  ...
  if (btnLimparAntigos) {
    btnLimparAntigos.addEventListener('click', () => {
      const hoje = hojeISO();
      if (!confirm('Isso vai remover da lista principal e do armazenamento todos os jogos com data anterior a hoje. Continuar?')) return;
  ...
  if (btnConfirmarLimparDia) {
    btnConfirmarLimparDia.addEventListener('click', () => {
      const dataEscolhida = $('#input-data-limpar-dia').value;
      if (!dataEscolhida) {
        mostrarToast('Escolha uma data para apagar.');
        return;
      }
      if (!confirm(`Isso vai remover todos os jogos com data ${dataEscolhida}. Continuar?`)) return;
