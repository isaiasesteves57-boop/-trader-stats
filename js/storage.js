...
const RadarStorage = (() => {
  ...
  // Importação inteligente: mesmo jogo/data acumula estratégias e entradas
  // em vez de criar cards duplicados.
  function addOrMergeOpportunity(op) {
    const list = getOpportunities();
    const key = (x) => `${(x.timeCasa || '').trim().toLowerCase()}|${(x.timeVisitante || '').trim().toLowerCase()}|${x.data || ''}`;
