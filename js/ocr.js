    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          function cropAndUpscale(sx, sw) {
            const alvoLargura = Math.min(2400, Math.max(1600, Math.round(sw * 1.45)));
            const canvas = document.createElement('canvas');
            canvas.width = alvoLargura;
            canvas.height = Math.round(height * (alvoLargura / sw));
            const ctx = canvas.getContext('2d');
            ctx.imageSmoothingEnabled = true;
            ctx.imageSmoothingQuality = 'high';
            ctx.drawImage(img, sx, 0, sw, height, 0, 0, canvas.width, canvas.height);
            return canvas.toDataURL('image/png');
          }

          const larguraEsq = meio - margem;
          const larguraDir = width - meio - margem;
          const dataUrlEsq = cropAndUpscale(0, larguraEsq);
          const dataUrlDir = cropAndUpscale(meio + margem, larguraDir);
          resolve([dataUrlEsq, dataUrlDir]);
        } catch (err) {
          reject(err);
        }
      };
